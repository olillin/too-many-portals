...
def create_portals(ctx: Context, config: ConfigOptions):
    ...
    for i, portal in enumerate(config.portals):
        ...
        for name, value in {
            ...
            ctx.meta["customportals"]["portal"][name] = value

        # Render functions
        path = f"portals/{dimension_a_id}/{dimension_b_id}"
        ctx.require(
            load(data_pack={
                f"data/customportals/functions/{path}": f'customportals/per_portal/functions/{"floor" if portal.floor_portal else "default"}'
            }),
            render(data_pack={
                "functions": ["customportals:" + path]
            })
        )
        ...
        if portal.floor_portal:
            ctx.data.functions[f"{NAMESPACE}:check/floor/{portal.activation}"].lines.append(
                f'execute if block ~-1 ~ ~ {block} run function customportals:portals/{dimension_a_id}/{dimension_b_id}/build/check_frame'
            )
            ctx.data.functions[f"{NAMESPACE}:break/check_floor"].lines.append(
                f'execute if entity @s[tag={dimension_a_id},tag={dimension_b_id}] unless predicate customportals:portals/{dimension_a_id}/{dimension_b_id}/created_frame run function customportals:break/break_floor'
            )
        else:
            ...
            ctx.data.functions[f"{NAMESPACE}:break/check_x"].lines.append(
                f'execute if entity @s[tag={dimension_a_id},tag={dimension_b_id}] unless predicate customportals:portals/{dimension_a_id}/{dimension_b_id}/created_frame_x run function customportals:break/break_x'
            )
            ctx.data.functions[f"{NAMESPACE}:break/check_z"].lines.append(
                f'execute if entity @s[tag={dimension_a_id},tag={dimension_b_id}] unless predicate customportals:portals/{dimension_a_id}/{dimension_b_id}/created_frame_z run function customportals:break/break_z'
            )
        ctx.data.functions[f"{NAMESPACE}:enter/identify"].lines.append(
            f'execute if entity @s[tag={dimension_a_id},tag={dimension_b_id}] as @e[type=#customportals:affectable,tag=customportals.affectable,tag=customportals.teleport,sort=nearest,limit=1] run function customportals:portals/{dimension_a_id}/{dimension_b_id}/travel/enter'
        )
        ctx.data.functions[f"{NAMESPACE}:update"].lines.insert(0,
            f'execute if entity @s[tag={dimension_a_id},tag={dimension_b_id}] run function customportals:portals/{dimension_a_id}/{dimension_b_id}/update'
        )
        ...
        if portal.floor_portal:
            ctx.assets.models[f"{NAMESPACE}:portal/{i+1}"] = Model({
                "parent": "customportals:portal/portal_floor",
                "textures": {
                    "portal": texture
                }
            })
            ...
            ctx.assets.models[f"{NAMESPACE}:portal/{i+1}_x"] = Model({
                "parent": "customportals:portal/portal_x",
                "textures": {
                    "portal": texture
                }
            })
            ctx.assets.models[f"{NAMESPACE}:portal/{i+1}_z"] = Model({
                "parent": "customportals:portal/portal_z",
                "textures": {
                    "portal": texture
                }
            })
